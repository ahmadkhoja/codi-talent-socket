...
class FilterSelected extends Component  {
  ...
  handleSubmit = (e) => {
        ...
        this.props.uploader.upload(image)
        this.props.socket.emit('student:signup',name,city,skills,availability,gender,imagename)
  }
      ...
      <div>
          <h3 className="filterSelected">{this.props.children}</h3>
            <form onSubmit={this.handleSubmit} encType="multipart/form-data">
              ...
              <label>
                Upload a photo:
                <input type="file" id="photo" name="photo" className="student-image"/>
              </label>
              <input type="submit" />
            </form>
      </div>
...
export default class StudentTalent extends Component {
  ...
  renderFilteredStudents() {
    ...
    const reactStudents = finalStudentList.map(
      (student) => <Student  OnClickAdd={() => this.AddStudentToList(student)} image={'/uploadedImages/'+student.imagename} {...student} key={student.name}/> 
    )
  ...
  renderSelectedStudents(){
    return this.state.userSelectedList.map(
        (student) => <StudentSelected  OnClickRemove={() => this.OnClickRemove(student)} OnClickAdd={() => this.AddStudentToList(student)}   image={'/uploadedImages/'+student.imagename} {...student} key={student.name}/> 
      )
   }
    ...
    <div>
       <div className="multipleFilters">
          { ( this.state.cityFilter || this.state.skillFilter || this.state.availableFilter|| this.state.genderFilter?
               null : <FilterSelected socket={this.state.socket} uploader={this.state.uploader}>
               No Filter Selected
            </FilterSelected>  ) 
          }
